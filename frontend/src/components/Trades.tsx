    ...
    <div className="flex flex-col h-full bg-[#121721] select-none">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-transparent">
        {trades.map((t) => (
          <div
            key={t.id}
            className="grid grid-cols-3 px-3 sm:px-4 py-0.5 font-mono text-[11px] sm:text-xs tabular-nums hover:bg-[#181e2b] transition-colors cursor-default"
          >
            <span
              className={`font-bold ${
            ...
            <span className="text-right text-[#848e9c] text-[10px] sm:text-xs font-medium">
              {fmtTime(t.timestamp)}
            </span>
          </div>
        ))}
